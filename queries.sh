...
echo "$($PSQL "select distinct(name) from teams inner join games on teams.team_id = games.winner_id where year=2014 and round='Eighth-Final' union select distinct(name) from teams inner join games on teams.team_id = games.opponent_id where year=2014 and round='Eighth-Final'")"
